function [sampleData, EDI, EDT, EDR, perc95] = runSampling_SCI_1(simData,numRuns,popSize,sampleSize,sampleInterval,tFinal,progress)

% INPUT
% simData: cell array of simulated incidence curves, generated from runSpread_SCI_1.m
...
% sampling run. Entries in the first column are (total) discovery incidences; entries in the
...
for i=1:numRuns
    ...
    while (detectionIndicator == 0 && sampleTime <= tFinal)
        ...
        numSymptomatic = selectedSimData{i,2}(sampleIndex); % Compute total number of symptomatic infected plants
        numCryptic = selectedSimData{i,3}(sampleIndex); % Compute total number of cryptic plants
        stateVec = ones(1,numSymptomatic); stateVec(P) = 0; % Create vector containing a 1 for every symptomatic plant and a 0 for every asymptomatic/healthy plant
        selectVec = randsample(stateVec,N); % Take a random sample of size N (without replacement) from this vector
        sympSample = sum(selectVec); % Total number of symptomatic infected plants in the sample
        if sympSample > 0 % Then an infected plant has been sampled
    ...
    sampleData(i,1) = numSymptomatic + numCryptic;
    sampleData(i,2) = sampleTime;
    sampleData(i,3) = samplingRound;
end
EDI = mean(sampleData(:,1)); % Expected total discovery incidence (symptomatic and asymptomatic)
